...
import { AlertCircle, ArrowLeft, CheckCircle2, Download, LoaderCircle, RefreshCw, RotateCcw, Search, Trash2, X } from 'lucide-react'
import Field from './ApplicationField'
import { DEPARTMENTS, getEquipmentFields, getRequestFields, OPERATIONS, OPERATION_FIELDS, TYPES as REGISTRATION_TYPES, USAGE_FIELDS } from './formConfig'
import { apiRequest, useActionSignal, useApiResource, useBeforeUnload } from './ledgerApi'
...
function getFields(form) {
  ...
  const selected = REGISTRATION_TYPES[form.application_type]
    ? [...getEquipmentFields(form.application_type), ...getRequestFields(form.application_type, form.operation_type)]
    : [...(type?.fields ?? []), ...(OPERATION_FIELDS[form.operation_type] ?? []), ...(USAGE_FIELDS[form.operation_type] ?? []), ...(type?.secondaryFields ?? [])]
...
function RecordHistory({ entries }) {
  ...
  return <section className="history-section"><div className="detail-section-head"><h2>変更履歴</h2><span>{entries.length}件</span></div><p className="help-text">過去の登録には、変更履歴がない場合があります。</p>
...
function RecordContent({ record, onChanged, onReload, onUnauthorized, onBack, onReadReference, registerLeaveGuard }) {
  ...
  const equipmentKeys = new Set(getEquipmentFields(form.application_type).map(([key]) => key))
  const equipmentFields = fields.filter(([key]) => equipmentKeys.has(key))
  const requestFields = fields.filter(([key]) => !equipmentKeys.has(key))
  ...
  const hasUnsavedInput = (editing && hasChanges) || (cancelStep && reason.length > 0)
  const changedEquipmentType = form.application_type !== record.application_type
  const clearRelatedLoan = changedEquipmentType || form.operation_type !== 'return' || String(form.details.management_number ?? '').trim() !== String(record.details?.management_number ?? '').trim()
  useBeforeUnload(hasUnsavedInput || busy)
  useEffect(() => registerLeaveGuard(() => {
    if (busy) return false
    return !hasUnsavedInput || window.confirm('保存していない入力を破棄して画面を移動しますか？')
  }), [registerLeaveGuard, hasUnsavedInput, busy])
  const setDetail = (key, value) => setForm((current) => ({ ...current, details: { ...current.details, [key]: value } }))
  const renderField = (field) => typeof form.details[field[0]] === 'object' && form.details[field[0]] !== null ? <label className="wide" key={field[0]}>{field[1]}（保存済みの詳細）<textarea value={JSON.stringify(form.details[field[0]], null, 2)} readOnly /><small>複合形式の値は保持されます。</small></label> : <Field key={field[0]} field={field} value={form.details[field[0]]} onChange={setDetail} />
  const leave = () => {
    if (busy || (hasUnsavedInput && !window.confirm('保存していない入力を破棄して台帳へ戻りますか？'))) return
    onBack()
  }
  const save = async (action, event) => {
    ...
    try {
      ...
      const body = action === 'update' ? { ...form, revision: record.revision, ...(changedEquipmentType && record.source_application ? { source_application: null } : {}), ...(clearRelatedLoan && record.related_loan ? { related_loan: null } : {}) } : { revision: record.revision, ...(action === 'cancel' ? { reason: reason.trim() } : {}) }
  ...
  return <>
    ...
    <section className="record-detail-card">
      <div className="record-title"><div><h1>{editing ? '登録内容を修正' : '登録内容を確認'}</h1><p className="help-text">{TYPES[record.application_type]?.label}・{OPERATIONS[record.operation_type]?.label} ／ 更新 {formatDate(record.updated_at || record.created_at)}</p></div>{record.is_cancelled ? <span className="cancelled-badge">取消済み</span> : !editing && <button className="button primary" onClick={() => setEditing(true)}>修正する</button>}</div>
      <div className="responsible-card"><span>登録責任者</span><strong>{record.entered_by_name || record.entered_by_email || '不明'}</strong><small>{record.entered_by_email}</small><p>登録日時：{formatDate(record.created_at)}</p></div>
      {(record.source_application || record.related_loan) && <div className="equipment-source"><p>参照した申請</p>{record.source_application && <p>機器情報：<button type="button" className="inline-button" disabled={busy || hasUnsavedInput} onClick={() => onReadReference(record.source_application)}>{record.source_application_reference}</button></p>}{record.related_loan && <p>参考の貸出：<button type="button" className="inline-button" disabled={busy || hasUnsavedInput} onClick={() => onReadReference(record.related_loan)}>{record.related_loan_reference}</button></p>}{editing && ((changedEquipmentType && record.source_application) || (clearRelatedLoan && record.related_loan)) && <p>機器種別・管理番号・処理の変更に伴い、対応しなくなる参照関係は保存時に解除されます。</p>}</div>}
      ...
      {editing ? <form onSubmit={(event) => save('update', event)}>
        <p className="help-text edit-guidance">修正内容は台帳とExcelに反映され、変更履歴に残ります。</p>
        <fieldset className="edit-fieldset" disabled={busy || conflict}>
          <section className="form-section">
            <h2 className="form-section-title">機器情報</h2>
            <div className="form-grid">
              <label>機器種別<select value={form.application_type} onChange={(event) => setForm({ ...form, application_type: event.target.value })}>{Object.entries(TYPES).map(([key, type]) => <option key={key} value={key}>{type.label}</option>)}</select></label>
              {equipmentFields.map(renderField)}
            </div>
          </section>
          <section className="form-section">
            <h2 className="form-section-title">今回の申請内容</h2>
            <div className="form-grid">
        ...
        <section className="review-section"><h2>機器情報</h2><dl className="review-list record-review">
          <div><dt>機器種別</dt><dd>{TYPES[record.application_type]?.label}</dd></div>
          {equipmentFields.map(([key, label]) => <div key={key}><dt>{label}</dt><dd>{displayValue(record.details?.[key])}</dd></div>)}
        </dl></section>
        <section className="review-section"><h2>今回の申請内容</h2><dl className="review-list record-review">
          <div><dt>処理</dt><dd>{OPERATIONS[record.operation_type]?.label}</dd></div>
          <div><dt>申請者氏名</dt><dd>{displayValue(record.applicant_name)}</dd></div>
          <div><dt>所属部署</dt><dd>{displayValue(record.department)}</dd></div>
          {requestFields.map(([key, label]) => <div key={key}><dt>{label}</dt><dd>{displayValue(record.details?.[key])}</dd></div>)}
          <div><dt>担当者メモ</dt><dd>{displayValue(record.notes)}</dd></div>
        </dl></section>
      </>}
    </section>
...
export default function LedgerWorkspace({ initialType = 'pc', initialRecordId = null, refresh, onRefresh, onUnauthorized, registerLeaveGuard }) {
  ...
  if (recordId) return <main className="workspace detail-workspace">
    ...
    {detail.loading ? <div className="empty" role="status"><LoaderCircle className="spin" />登録内容を読み込み中</div> : detail.error ? <><button className="text-button" onClick={() => setRecordId(null)}><ArrowLeft />台帳へ戻る</button><Alert>{detail.error} <button className="inline-button" onClick={onRefresh}>再読み込み</button></Alert></> : detail.data && <RecordContent key={`${detail.data.id}-${detail.data.revision}-${refresh}`} record={detail.data} onChanged={changed} onReadReference={(id) => { setRecordId(id); setFlash(null) }} onUnauthorized={onUnauthorized} registerLeaveGuard={registerLeaveGuard} onReload={onRefresh} onBack={() => { setRecordId(null); setFlash(null) }} />}
  </main>

  return <main className="workspace ledger-workspace">
    <section className="ledger-heading"><div><h1>台帳を確認</h1></div></section>
    <div className="ledger-types" role="group" aria-label="機器種別">{Object.entries(TYPES).map(([key, item]) => <button key={key} className={type === key ? 'selected' : ''} aria-pressed={type === key} disabled={Boolean(busy)} onClick={() => pickType(key)}>{item.label}</button>)}</div>
    {flash && <Alert kind={flash.warning ? 'error' : 'success'}>{flash.text}</Alert>}
    ...
    <section className="records-card ledger-card">
      <div className="section-head"><div><h2>{TYPES[type].label}の台帳</h2></div><button className="button primary" disabled={Boolean(busy) || ledger.loading || Boolean(ledger.error)} onClick={() => ledgerAction('download')}>{busy === 'download' ? <LoaderCircle className="spin" /> : <Download />}台帳全体をExcelで取得</button></div>
