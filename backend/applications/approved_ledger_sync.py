...
DETAIL_COLUMNS = {
    'pc': (('機種・端末名', 'device_name'),),
    'memory': (('機器名', 'device_name'), ('容量', 'capacity')),
    'lan': (('機器種別', 'device_type'), ('機器名', 'device_name')),
    'phone': (('OS', 'os'), ('機種', 'model_name'), ('容量', 'storage')),
    'other': (('転記内容', '_all'),),
}
...
def sync_approved_ledger(application_type):
    ...
    if columns is None:
        raise ValueError(f'未対応の機器種別です: {application_type}')
    ...
    worksheet.append([
        ...
        '登録担当者',
        '登録日時',
        *(label for label, _key in all_columns),
        '担当者メモ',
    ])
